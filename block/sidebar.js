function fillHeadingTree(array, newNode) {
  ...
  fillHeadingTree(currNode.children, newNode);
}
...
function toggleSubtree(togglerClassName, subtreeClassName) {
  ...
  if (
    !headingTree__togglerEl.classList.contains("heading-tree__toggler_opened")
  ) {
  ...
  for (const headingTree__subtreeEl of document.querySelectorAll(
    `.${subtreeClassName}`
  )) {
...
function fillHeadingTreeEl(array, depth, data) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (currNode.children.length !== 0) {
      ...
      headingTree__togglerEl.classList.add(
      ...
      headingTreeEl.appendChild(
        fillHeadingTreeEl(currNode.children, depth + 1, data)
      );
    } else {
      headingTree__nodeEl.appendChild(headingTree__linkEl);
    }
...
function addHeadingTreeEl(headings, sidebarLayoutEl, prefix) {
  ...
  sidebarLayoutEl.appendChild(fillHeadingTreeEl(tree, 0, data));
}
...
function addMobileSidebar(headings) {
  ...
  mobileSidebar__togglerEl.onclick = function () {
    const mobileSidebar__layoutEl = document.querySelector(
    ...
    for (const mobileSidebar__togglerIconEl of document.querySelectorAll(
  ...
  mobileSidebar__layoutEl.classList.add(
    "sidebar__layout",
    "mobile-sidebar__layout"
  );
...
function addDesktopSidebar(headings) {
  ...
  desktopSidebar__layoutEl.classList.add(
    "sidebar__layout",
    "desktop-sidebar__layout"
  );
